...
class Gh_Crop:
    ...
    def info():
        return {
            "name": "gh_crop",
            "description": "Crop matrix selector for GH"
        }
    ...
    def input_type():
        return {
            ...
            "yield impact coefficient": {
                "description": "Yield impact coefficient from energy calculations",
                "default": 1,
                "type": "decimal"
            }
        }
    ...
    def compute(args, quantize_output=False):
        ...
        result = dict()
        inputs = parse_inputs(Gh_Crop.input_type(), args)

        output_type = Gh_Crop.output_type()
        ...
        for input_row in inputs['crops']:
            yield_coefficient = inputs['yield impact coefficient'] * \
                input_row['grow system yield factor']
            s_start = timeit.default_timer()
            ...
            line['substrate req'] = row['substrate_requirement'] * \
                line['area'] * line['cycles']
            line['yield'] = yield_coefficient * row['lbs_per_plant_per_cycle'] * \
