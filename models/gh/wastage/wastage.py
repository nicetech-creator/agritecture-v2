...
class Gh_Wastage:
    ...
    def info():
        return {
            "name": "gh_wastage",
            "description": "Wastage model for GH"
        }
    ...
    def input_values(input_name):
        if input_name == "grower experience":
            return fetch_all(
                "SELECT id, description FROM gh_grower_experience")
    ...
    def compute(args, quantize_output=False):
        ...
        result = dict()
        inputs = parse_inputs(Gh_Wastage.input_type(), args)

        output_type = Gh_Wastage.output_type()
        ...
        grower_experience_row = fetch_one(
            "SELECT base_wastage w FROM gh_grower_experience WHERE id=%(element_id)s",
            query_inputs)
