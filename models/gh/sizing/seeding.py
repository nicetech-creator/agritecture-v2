...
class Gh_Sizing_Seeding:

    @staticmethod
    def info():
        return {
            "name": "gh_sizing_seeding",
            "description": "Seeding area allocation for GH"
        }
    ...
    def compute(args, quantize_output=False):
        ...
        result = dict()
        inputs = parse_inputs(Gh_Sizing_Seeding.input_type(), args)

        output_type = Gh_Sizing_Seeding.output_type()
        ...
        set_if_unset(
            ...
            "seeding bedspace needed non-microgreens",
            math.ceil(
                result["seeding bedspace per week non-microgreens"] *
                3))
