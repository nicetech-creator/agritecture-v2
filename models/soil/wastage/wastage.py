...
class Soil_Wastage:
    ...
    def info():
        return {
            "name": "soil_wastage",
            "description": "Wastage model for Soil"
        }
    ...
    def input_values(input_name):
        if input_name == "grower experience":
            return fetch_all(
                "SELECT id, description FROM soil_grower_experience")
    ...
    def compute(args, quantize_output=False):
        result = dict()
        inputs = parse_inputs(Soil_Wastage.input_type(), args)

        output_type = Soil_Wastage.output_type()
        ...
        grower_experience_row = fetch_one(
            "SELECT base_wastage w FROM soil_grower_experience WHERE id=%(element_id)s",
            query_inputs)
