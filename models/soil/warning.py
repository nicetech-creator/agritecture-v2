...
class Soil_Warning:
    ...
    def info():
        return {
            "name": "soil_warning",
            "description": "Warning list for Soil"
        }
    ...
    def compute(args, quantize_output=False):
        result = dict()
        inputs = parse_inputs(Soil_Warning.input_type(), args)

        output_type = Soil_Warning.output_type()
        ...
        for row in inputs['crops']:
            ...
            crop_data_row = fetch_one(
                "SELECT id, crop_type, default_price_per_lb_market, default_price_per_each_market FROM crops WHERE id=%(id)s", {
                    "id": crop_id})
