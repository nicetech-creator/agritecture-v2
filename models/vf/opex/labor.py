...
class Vf_Labor:
    ...
    def info():
        return {
            "name": "vf_labor",
            "description": "Labor schedule selector for VF"
        }
    ...
    def compute(args, quantize_output=False):
        ...
        result = dict()
        inputs = parse_inputs(Vf_Labor.input_type(), args)
        advanced_inputs = arr2dic(inputs["advanced"])
        output_type = Vf_Labor.output_type()
        ...
        bedspace_row = fetch_one(
            "SELECT bedspace_per_laborer FROM vf_labor WHERE automation=%(automation)s",
            query_inputs)
